...
function SearchBox() {
  ...
  useEffect(() => {
    ...
    const enterKey = (e) => {
      ...
      setShowResult(true);
      if (e.keyCode === 13) {
        navigate(debouncedValue !== "" ? `/${debouncedValue}` : "");
        setShowResult(false);
        setInputValue("");
      }
    };
          ...
          <div className={cs("search-result")} tabIndex="-1" {...attrs}>
            <Wrapper>
              {searchResult.length > 0 ? (
                <>
                  <h4
                    className={cs("search-title")}
                  >{`Kết quả cho '${debouncedValue}'`}</h4>
                  <Link
                    to={debouncedValue !== "" ? `/${debouncedValue}` : ""}
                    onClick={() => {
                      setShowResult(false);
                      setInputValue("");
                    }}
                ...
                <p
                  className={cs("noResult")}
                >{`Không có kết quả cho '${debouncedValue}'`}</p>
              )}
            </Wrapper>
          </div>
        )}
